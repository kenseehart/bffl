...
`bffl` is an easy to use high performance bit field protocol framework for working with packed binary data. It's ideal for things like verilog interfaces, and any situation where you are working with arbitrarily sized bitfields, and you need precise convenient control of the bit arrangement. Your protocol is expressed concisely with any composition of ints, structs, arrays, and user defined types.


# Comparison to [ctypes](https://docs.python.org/3/library/ctypes.html)

While bffl and ctypes have some similarities, the primary purpose is different. Both bffl and ctypes are able to easily encode/decode binary data in pure python such as c structs without a generated wrapper. The differ in that `ctypes` maps to c structs while `bffl` maps to bit vectors.

| **tool** | **model** | **primary purpose** | **implementation** |
|----|--------|---|----|
| ctypes | C/C++ types | interface with C/C++ code, model C/C++ datatypes | python, C++ |
| bffl | bit fields | model arbitary bit alligned datatypes and interfaces | python |

* Use ctypes if you need to interface with C/C++ code.
* Use bffl if you need to have full control over a bit aligned binary protocol.
* Use both ctypes and bffl if you need both. The can work together.

# Comparison to bitfields in C++

Unfortunately, the core philosophy of `C/C++` is that the language ultimately decides how to allocate bits for optimal performance, and the language thinks it knows better than you how to do this. For example, C++ respects byte or word boundaries. If you have a good reason to control bit alignment and C++ doesn't permit your choice, you have to write methods to explicitly extract the desired bits. So bitfields in C are inadequate if you want to really control bitwise allocation and have predictable results, such as assuming the size in bits of a struct is the sum of the sizes of it's members. This level of control is particularly relevant for things like protocol implementations and verilog. Now with `bffl`, bitfields behave exactly asyou expect.

In other words, `bffl` prioritizes explicit control over respect over word or byte boundaries. A struct with a 5 bit integer and a 13 bit integer is an 18 bit struct, and an array of 5 18 bit structs is exactly 90 bits. Of course you may choose to design your structures to respect word boundaries, and there may be use cases where doing so can improve performance, but that's entirely up to you.
...
In C++, bit field allocation is implementation defined. Usually word boundaries are respected, and the total size of a structure is usually implicitly padded to a multiple of 8 bits. This is the primary reason why `ctypes` is not suitable for certain use cases for which `bffl` was designed, such as verilog interface testing, where explicit control of bit allocation is a requirement.

In `bffl`, the size of a `struct` is simply the sum of the size of it's members, which are allocated consecutively without implicit padding. Fields may even straddle word boundaries. The intent is to give the programmer full control and predictability with respect to bit allocation. If you allocate an array of 7 7-bit integers, it will occupy precisely 49 bits: `uint(7)[7].size_ == 49`. Also, there is no upper bound on the size of a field. Note that the `_size` attribute is the size in bits, whereas the C `sizeof()` operator is the size in bytes. 

Although it can be argued that byte/word alignment is usually a good idea, `bffl` is intentionally agnostic about such things, and gives complete control over alignment policies to the developer. However, if desired, it would not be difficult to implement a custom allocation policy to add padding as needed.
...
Python integers are unbounded, which means a data record of any size can be encoded as a single integer, and bffl operates on these unbounded integers.
...
The bffl framework is extensible, the the following is not comprehensive.
...
In `bffl`, performance is acheived by performing nearly all symbolic processing at interface allocation time, prior to binding data, and typically outside the main loop. So a python only application is pretty quick. Inside the main loop, a bound field is usually computed with nothing more than a `shift-and` operation.
...
If you need performance that exceeds typical C++, we can help. See [high performance query module](https://github.com/kenseehart/bffl/issues/2) This would take about 40 hours of effort, hopefully with the support of a patron. Let me know if this is important to you. 
...
By default, `bffl` uses python semantics for indexing and slicing because it seems preferable to use the conventions of the native language. However, in order to make system verilog protocols more readable, we have a `svreg` type that implements System Verilog slice semantics.
...
- Parse System Verilog structs to **bffl**
...
- numpy + bcolz integration
  ...
  - bcolz and bffl have excellent affinity
...
- Alternate customizable packing
  - Although `bffl` bitfields are packed by default, sometimes you need unpacked structures
